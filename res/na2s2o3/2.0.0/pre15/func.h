...
class f1:public mainfunc{
	public:
		...
		void advancement(int id){
			...
			dgncx.SetPos(81,3);nowee[id]=1;cout<<"已达成彩蛋:"<<f1_EET()<<"/"<<eastereggt;
		...
		void ending(int id){
			...
			dgncx.SetPos(80,4);cout<<"┃                              ┃";
			dgncx.SetPos(80,5);cout<<"└──────────────────────────────┚";
			dgncx.SetPos(81,3);end[id]=1;cout<<"已达成结尾:"<<f1_ENDT()<<"/"<<5;
...
class mainfunc:public dgncx{
	...
	public:
		...
		void home(){
			dgncx hhhhh;
			srand(time(0));
			while(endgame!=1){cl();dgncx.color(15);
				switch(f1.home()){
					case '1':f1.f1_main();cl();break;
					//case '2':f2();cl();break;
					//case 'i':fi();cl();break;
...
//dgncx function
void color(const int a) { SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), a); }
void dgncx.SetPos(COORD a) { HANDLE out = GetStdHandle(STD_OUTPUT_HANDLE); SetConsoleCursorPosition(out, a); }
void dgncx.SetPos(int i, int j) { COORD pos = { (SHORT)i, (SHORT)j }; dgncx.SetPos(pos); }
void clear(int x, int y) {
	swap(x, y);
	dgncx.SetPos(x+1, y + 1);
	putchar(' ');
}
void clear(const pair<int, int> p) { clear(p.first, p.second); }
void print(int x, int y, const int col = 2) {
...
void fi(){
	...
	color(6);dgncx.SetPos(6,2);cout<<"[R]";
	color(15);while(tolower(_getch())!='r'){}
}
void f2(){
	while(endgame!=1){
