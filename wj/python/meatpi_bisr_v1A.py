...
A,B,C,E,I,K,L,M,N,O,P,Q,R,S,T,U,W,X,Y,Z=abs,bin,chr,eval,input,t.bk,len,min,open,ord,print,int,r.randint,str,t.fd,tm.sleep,range,max,180,90
if os.name=='nt':
 _=os.system('cls')
else:
 _=os.system('clear')
P('Copyright (c) 2022 Python Source Code Collecting Org.\nAll Rights Reserved.\n')
class Dt:
 try:
  nums66,et47,wheel62,hfms,block27,blockcol27,map135,map235,map138,a23,refl62,inits2293=E(N('Meatpi_Data.dat','r').read())
 except:
  P('WARNING\nData file not found or not available.\nOffline Mode has been enabled automatically.')
class Sz:
 try:
  ls=E(N('Meatpi_Settings.dat','r').read())
  if L(ls)!=6:
   E(N('Meatpi_Data.dat','r').read())
 except:
  ls=[1,None,1,0.6,I('Username (will be saved):'),None]
  N('Meatpi_Settings.dat','w').write(S(ls))
 name=['sound','deleted','output clearing','time gap','username','deleted']
def clear():
 if Sz.ls[2]==0:
  return 0
 if os.name=='nt':
...
def mst():
 ...
 return tim
def bzst():
 tim=S(Q((tm.time()-1655740800)/1836731*10000000))
 ln=L(tim)
 if ln==8:
  tim=tim[0:3]+'.'+tim[3]+'.'+tim[4]+':'+tim[ln-3:ln]
 elif ln==9:
  tim=tim[0:3]+'.'+tim[3:5]+'.'+tim[5]+':'+tim[ln-3:ln]
 else:
  tim=tim[0:3]+'.'+tim[3:5]+'.'+tim[5:ln-4]+':'+tim[ln-4:ln]
 return tim
class c64:
  vis=[]
...
def op66(modes,x,y,ds,dopr):
 ...
 for i in W(14):
  ...
  if modes[i]==3:
   cor[i]=1-cor[i]
 c1=lambda x:Q(cor[x])*'_'+(1-Q(cor[x]))*' '
 c2=lambda x:Q(cor[x])*'|'+(1-Q(cor[x]))*' '
 if dopr:
  P(' '+c1(0)+'   '+c1(7))
  P(c2(1)+c1(3)+c2(2)+' '+c2(8)+c1(10)+c2(9))
  P(c2(4)+c1(6)+c2(5)+' '+c2(11)+c1(13)+c2(12))
 return cor
def read(k):
 P()
 P(Sz.name[k])
...
ax²+bx+c=0''')
 ...
 if a==0 and b==0 and c==0:
  P('x = 0')
 if b*b-4*a*c < 0:
  P('x =', '(', -b, '+', sqrt(-(b*b-4*a*c)),'i', ')', '/', 2*a, 'or', '(', -b, '-', sqrt(-(b*b-4*a*c)), 'i', ')', '/', 2*a)
 else:
  P('x =', (-b+sqrt(b*b-4*a*c))/a/2, 'or', (-b-sqrt(b*b-4*a*c))/a/2)
def zp15():
 n=Q(I('请输入想分解的数:'))
 if n<1:
  P('无法分解')
...
def zp27():
 ...
 while 1:
  ...
  for i in W(3):
   ...
   P()
  col=Q(I('放在第几列?'))
  while col+blockcol[typ]>9 or col<1:
   col=Q(I('无效列数,请重新输入:'))
  clear()
  brk=1
  place=15
...
def zp72():
 ...
 while 1:
  cnt+=1
  sx,sy='x'+S(cnt)+':','y'+S(cnt)+':'
  x,y=E(I(sx)),E(I(sy))
  p.append([x,y])
  i=cnt-1
  for j in W(cnt-1):
   try:
    k=(p[i][1]-p[j][1])/(p[i][0]-p[j][0])
    b=p[i][1]-p[i][0]*k
    midx=(p[i][0]+p[j][0])/2
    midy=(p[i][1]+p[j][1])/2
    k2=-1/k
    b2=midy-k2*midx
    P('P%d & P%d'%(i+1,j+1))
    P('  连线解析式: y = %.3fx + %.3f'%(k,b))
    P('  与x轴交点: (%.3f, 0)'%(-b/k))
    P('  与y轴交点: (0, %.3f)'%(b))
    P('  连线中点: (%.3f, %.3f)'%(midx,midy))
    P('  中垂线解析式: y = %.3fx + %.3f'%(k2,b2))
    P()
   except:
    pass
def zp76():
...
70 铁板                   71 推箱子  ***''')
   if bm=='2':
    P('''\
01 普通计算器             02 质数判断/生成器
03 等差数列求和           04 化简分数
05 十进制变几进制         06 几进制变十进制
07 最小公倍数             12 最大公因数
13 解二元一次方程         14 解一元二次方程
15 质因数分解             20 计算π
30 生成随机数             31 计算平均数
33 帕斯卡三角形           63 平方根式无理数计算器
72 几何结合器''')
   if bm=='3':
    P('''\
08 算24                   22 画板
